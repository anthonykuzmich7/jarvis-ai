...
/* Gmail — uses the official icon downloaded to /public */
...
const SYNC_STATES = [
  { id: "slack",   Mark: SlackMark,   label: "Reading Slack messages..."  },
  { id: "gmail",   Mark: GmailMark,   label: "Catching up on email..."    },
  { id: "granola", Mark: GranolaMark, label: "Indexing meeting notes..."  },
  { id: "zoom",    Mark: ZoomMark,    label: "Processing recordings..."   },
] as const;

/* ─── Jarvis face (light — baby-blue with dark eyes for dark panel) ─ */
...
const PANEL_BG = "rgba(17, 17, 20, 0.97)";
const BADGE_BG = PANEL_BG; // badge border matches panel bg

function OverlayPanel({ live }: { live: boolean }) {
  const reduce = useReducedMotion();
  const [syncIdx, setSyncIdx] = React.useState(0);

  React.useEffect(() => {
    if (!live || reduce) return;
    const id = setInterval(() => {
      setSyncIdx((i) => (i + 1) % SYNC_STATES.length);
    }, CYCLE_MS);
    return () => clearInterval(id);
  }, [live, reduce]);

  const current = SYNC_STATES[syncIdx];
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow:
          "0 40px 100px rgba(0,0,0,0.32), 0 12px 32px rgba(0,0,0,0.2), 0 0 0 1px rgba(255,255,255,0.04)",
      }}
    >
      {/* Traffic lights */}
      <div className="absolute left-4 top-[14px] flex gap-1.5">
        <div className="h-[9px] w-[9px] rounded-full" style={{ background: "#ff5f57" }} />
        <div className="h-[9px] w-[9px] rounded-full" style={{ background: "#febc2e" }} />
        ...
        >
          ...
          <div className="relative flex h-[42px] flex-1 items-center overflow-hidden px-3.5 sm:px-5">
            <AnimatePresence mode="wait">
              ...
              >
                <div
                  className="flex h-[20px] w-[20px] shrink-0 items-center justify-center overflow-hidden rounded-[5px] bg-white p-[3px]"
                ...
                <motion.span
                  className="h-[6px] w-[6px] shrink-0 rounded-full bg-[#4d9bf0]"
                  animate={live && !reduce ? { opacity: [1, 0.2, 1] } : undefined}
      ...
      {/* Footer */}
      <div
        className="flex items-center justify-center gap-2 border-t px-6 py-[10px]"
        style={{ borderColor: "rgba(255,255,255,0.07)" }}
      >
        {["⌘", "J"].map((k) => (
          <span
            key={k}
            className="rounded px-[7px] py-0.5 text-[11px] text-white/35"
            style={{
        ...
        <span className="text-[11px] text-white/25">to search or command</span>
      </div>
    </div>
  );
}
...
export function JarvisOverlaySection() {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 70% 55% at 50% 60%, rgba(119,126,255,0.07) 0%, transparent 100%)",
        }}
      />
      ...
      >
        ...
        >
          {["⌘", "J"].map((k) => (
            <span
              key={k}
              className="flex h-8 w-8 items-center justify-center rounded-lg text-sm font-medium text-graphite"
              style={{
                background: "rgba(28,26,23,0.07)",
                border: "1px solid rgba(28,26,23,0.14)",
                boxShadow: "0 2px 0 rgba(28,26,23,0.1)",
              }}
            >
              {k}
            </span>
        ...
        >
          <h2 className="text-balance font-display text-3xl font-semibold leading-[1.15] tracking-[-0.64px] text-foreground sm:text-4xl">
            One shortcut. Every app. Always in sync.
          </h2>
          <p className="mt-4 text-pretty text-[17px] leading-relaxed text-muted-foreground">
            Press ⌘J anywhere. Jarvis has already caught up with your Slack, Gmail, Granola, and Zoom — before you even ask.
          </p>
        </motion.div>
